...
private let logger = Logger(subsystem: "app.modus", category: "OnDeviceDJBrain")
...
actor OnDeviceDJBrain: DJBrainProtocol {
    ...
    private static func sanitizeTransition(_ raw: String) -> String? {
        ...
        // Quoted lyric block heuristic: reject only if the quoted span exceeds
        // 20 characters. Short quotes (e.g. song title references like
        // "From \"Blinding Lights\" into...") are legitimate segues.
        if let firstQuote = result.firstIndex(of: "\""),
           let lastQuote = result.lastIndex(of: "\""),
           firstQuote != lastQuote,
           result.distance(from: firstQuote, to: lastQuote) > 20 {
            return nil
        }
