...
final class VectorAffinityEngine {
    ...
    static func applyFeedback(profile: inout UserTasteProfile, track: CachedTrack, playbackRatio: Double) {
        ...
        if playbackRatio >= 0.90 {
            profile.energyPreference += alpha * (track.energy - profile.energyPreference)
            profile.acousticnessPreference += alpha * (track.acousticness - profile.acousticnessPreference)
            profile.valencePreference += alpha * (track.valence - profile.valencePreference)
            profile.targetBPM += alpha * (track.bpm - profile.targetBPM)
            ...
            profile.energyPreference = max(0.0, min(1.0, profile.energyPreference))
            profile.acousticnessPreference = max(0.0, min(1.0, profile.acousticnessPreference))
            profile.valencePreference = max(0.0, min(1.0, profile.valencePreference))
        }
