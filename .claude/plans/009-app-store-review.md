...
- [x] **E1. Verify `TransitionManager.executeTransition(isEnabled: false)` is a true no-op** 🟢 sim — code path: `guard isEnabled else { return }` at line 45 returns immediately. No LLM, no TTS, no audio. **Also added defensive `isPro` gate on `preRenderTransition`** (no current callers, but protects future code). *Target: `TransitionManager.swift` — DONE 2026-06-07.*
- [x] **E2. Verify Pro-only UI controls on Free tier** 🟢 sim — `SearchView.StationOptionsSheet` (line 157-159) hides the `DJ Arc (Pro)` toggle entirely when `activeTier == .freeTier`. No silent no-op. *Target: `SearchView.swift` — DONE by inspection.*
- [ ] **E3. Verify Pro-only "Recent Stations" entry** 🟢 sim — gated on F4 implementation. *Target: `RadioView.swift` or sibling — DEFER to F.*
- [x] **E4. Add a Restore Purchases button** 🟡 sim-degraded — `PaywallSheet` is reachable from the tier badge in `RadioView`. It has a Restore Purchases button that calls `AppStore.sync()`. Verified in sim: tapping it opens the system Apple ID sign-in sheet (expected in sim with no signed-in iCloud). Product display card is empty in sim because `Product.products(for:)` returns empty when no real product is configured (a sim-only gap, not a code bug); the "Start Free Trial" button is hidden in that case. Verified on sim: paywall sheet surface, Restore button, error fallback. *Target: `SubscriptionManager.swift` + `RadioView.swift` — DONE 2026-06-07. Real-device gate before H6 re-verifies with a real product + sandbox Apple ID.*

### Milestone F — Station memory (Pro tier) — implementation (start now, mostly 🟢 sim, one 🟡 CloudKit cross-device)

- [x] **F1. Add `RecentStation` SwiftData model** 🟢 sim — new file `EchoDJ/Data/Models/RecentStation.swift` with `id, seedTrackID, seedTitle, seedArtist, createdAt, trackCount`. Registered in `AppEnvironment.modelContainer` schema. Wired into `generate-xcodeproj.py` SWIFT_FILES. CloudKit-private via the existing `cloudKitDatabase: .automatic` config. *Target: new file + `AppEnvironment.swift` + `generate-xcodeproj.py` — DONE 2026-06-07.*
- [x] **F2. Persist on station start** 🟢 sim — `StationQueueManager.generateStation` now calls `recordRecentStation(seed:trackCount:)` after `logStationSession`. Persistence happens unconditionally; the UI only surfaces it for Pro. *Target: `StationQueueManager.swift` — DONE 2026-06-07. End-to-end verification (tapping a cached track in Search → RecentStation row) requires UI automation that the sim tool doesn't expose; cross-checked by code review and successful build.*
- [x] **F3. Cooldown write on hard/soft/full play** 🟢 sim — `TelemetryCollector.applySkipFeedback` already writes cooldowns on skip (pre-existing, 24h for soft, 7d for hard). Added cooldown write to `recordFullPlay` (12h cooldown). Wired `recordFullPlay` into `RadioView.startProgressTimer` so track transitions are recorded. *Target: `TelemetryCollector.swift` + `RadioView.swift` — DONE 2026-06-07.*
- [x] **F4. Pro-gated "Recent Stations" UI entry** 🟢 sim — "Recent" button added to `RadioView` next to the tier badge. On Free: lock icon, taps route to paywall. On Pro: clock icon, opens `RecentStationsSheet` with `@Query` sorted by `createdAt` desc, empty state with onboarding copy. *Target: `RadioView.swift` — DONE 2026-06-07. Verified on sim: button present, Free tap routes to paywall.*
- [ ] **F5. CloudKit cross-device sync verification** 🟡 sim-degraded — schema is in place; SwiftData container is configured with `cloudKitDatabase: .automatic`. Single-device sim verification is the SwiftData write (already exercised in F2). True cross-device sync needs two real devices with the same iCloud account; deferred to the real-device gate before H6. *Target: `AppEnvironment.swift` schema config + CloudKit dashboard.*

### Milestone G — Info.plist, privacy, AI disclosure (start now, all 🟢 sim — docs only)

- [x] **G1. Create `PrivacyInfo.xcprivacy`** 🟢 sim — `EchoDJ/Resources/PrivacyInfo.xcprivacy` declares `NSPrivacyTracking: false`, no collected data types, no accessed APIs (we use SwiftData + CloudKit, both exempt). Wired into the bundle via `generate-xcodeproj.py` RESOURCE_FILES. Also added `NSAppleMusicUsageDescription` and `ITSAppUsesNonExemptEncryption = false` to `Info.plist` (both required for music apps and encryption export compliance). *Target: new file + `Info.plist` + `generate-xcodeproj.py` — DONE 2026-06-07.*
- [x] **G2. Draft `docs/app-store/metadata.md`** 🟢 sim — full App Store Connect copy drafts: name, subtitle, promotional text, description (≤4000), keywords (≤100), What's New, support/privacy/marketing URLs, privacy nutrition label answers, AI-Generated Content disclosure, export compliance, age rating (4+), demo Apple Music account notes, App Review contact, pre-submit sanity checklist. *Target: new file — DONE 2026-06-07.*
- [x] **G3. Draft `docs/app-store/rejection-playbook.md`** 🟢 sim — top 6 first-rejection reasons for music apps (MusicKit capability, Restore Purchases, privacy manifest, AI content rights, crash on launch, misleading subscription copy) with pre-built defenses, evidence to attach, and a general response template. *Target: new file — DONE 2026-06-07.*
